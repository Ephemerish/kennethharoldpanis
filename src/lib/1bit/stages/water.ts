 ...
 *   4. Clean up stray single-cell noise, then best-match-autotile the mask into
 *      water tiles with proper coasts.
 *
 * Writes `ctx.water` / `ctx.blocked` so later stages avoid water.
 */

import { NATURE_WATER_TILES, SOLID_TILE } from "../bountiful-bits";
...
/** Stamp a filled disc of the given radius into the water mask. */
function stampDisc(water: Uint8Array, cols: number, rows: number, cx: number, cy: number, r: number) {
...
function carveWater(ctx: WorldCtx) {
